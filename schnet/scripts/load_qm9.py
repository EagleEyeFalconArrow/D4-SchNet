...
def load_atomrefs(at_path):
    ...
    try:
        ...
        logging.error("%s HTTP Error: at url %s", err.code, at_url)
        return False
    except URLError as err:
        logging.error("%s URL Error: at %s", err.reason, at_url)
        return False
...
def load_data(dbpath):
    ...
    try:
        ...
        logging.error("%s HTTP Error: at %s", err.code, url)
        return False
    except URLError as err:
        logging.error("%s URL Error: at %s", err.reason, url)
        return False
    ...
    with connect(dbpath) as con:
        for i, xyzfile in enumerate(os.listdir(raw_path)):
            ...
            with open(xyzfile, 'r',encoding='utf-8') as f:
                ...
                with open(tmp, "wt",encoding='utf-8') as fout:
                    for line in lines:
